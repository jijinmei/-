...
<table class="layui-table margin0" id="laytable" style="table-layout:fixed;width:100%;"  v-if="datas!='no'">
      ...
      <tbody>
        <tr v-for="(item,index) in datas" >
          ...
          <td class='padding0'>{{item.reportInfo!=undefined?item.reportInfo.section:item.section}}</td>
          <td class='padding0'>
          <!-- @click="goapps(item.reportInfo.objectId)" -->
           <!-- <a class="c36c748 cursor" >{{item.reportInfo!=undefined?item.reportInfo.title:''}}</a> -->
             <a class="c36c748 cursor" @click="gotie(item.reportInfo.objectId)">{{item.title}}</a>
          <br>{{item.remark}}
          </td>
...
export default {
  ...
  methods:{
      ...
      $.get(Boss1+'section',function(data){
        if(data.status){
          ...
          for (var i = 0; i < aa.length; i++) {
            that.statusItem.push(aa[i].title)
          }
            console.log( that.statusItem)
        }
      })
   },
     goapps(objectId){
    ...
    statusclick(index,e){
      ...
      this.section=(index==0?'其他':this.statusVal)
...
https(page){
        ...
        that.$axios.get('boss/report', {
            params: {
            'searchkey':that.searchkey,'page':page,'section':that.section,'limit':limit10,'sessiontoken':sessiontoken
            }
          })
          .then(function (data) {
            if(data.status){
