...
<!-- 返回 -->
        <div class="h70 relative f18 c999999">
            ...
            <!-- 创建 -->
            <span @click="sendxin"  class="czjz R00 f18 cffffff w70 h40 lh40 center cursor" :class="{bg36c748:title!='',bg999999:title==''}">保存</span>
            <!-- 修改 -->
...
<form class="layui-form pT35" action="" onsubmit='return false'>
  <div class="layui-form-item">
    <label class="layui-form-label f18 c222222 w40">標題</label>
    <div class="layui-input-block mL70">
      <input v-model="title" type="text" name="title" required  lay-verify="required" placeholder="请输入標題" autocomplete="off" class="layui-input f18 c222222" style="width:690px;">
    </div>
  </div>
  </form>
<!--内容  -->
...
// 后台后期就会把编辑器里的图片转载到 pics 

// if(){
  // formData.append("pics", "http://lc-UIci50Wq.cn-n1.lcfile.com/ec1c8f379b8db7f0dfe3.png");
// }
...
editor.customConfig.uploadImgHooks = {
  ...
  goback(){
    if(this.title){
       ...
       var layer = layui.layer;
            layer.confirm('離開本頁面將清空當前頁面內容，<br>確認離開？',{
              ...
              yes: function(index, layero){
                ...
                window.sessionStorage.removeItem('xiaoxi7X')
                // that.$store.state.imgblob=[]
                // that.$store.state.imgUrl=[]
                 // that.$router.back(-1) 
              that.$router.push({name:'xiaoxi7',params:{'index':7}})   
      ...
      // this.$router.back(-1)
       this.$router.push({name:'xiaoxi7',params:{'index':7}})
    }
  ...
  mounted() {      
    ...
    // 如果是修改的，就將内容全部读取回来
    if(this.objectId!='no'){
       var bb=JSON.parse(window.sessionStorage.getItem('xiaoxi7X'))
