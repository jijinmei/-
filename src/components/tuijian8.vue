...
<table class="layui-table margin0" id="laytable" style="table-layout:fixed;width:100%;"  v-if="datas!='no'">
      <thead>
        <tr  style="background-color:#ebfceb;">
          ...
          <th class="padding0" width='317'>標題</th>
...
.always(function() {
            ...
            layer.confirm('確認刪除?',{
              ...
              yes: function(index, layero){
                ...
                headers:{'token':sessiontoken},
                 type: "POST",
                 url: Boss+'boss/article/'+that.objectIds+'/recommend',
                 data:{'action':0},
                 dataType: "json",
    ...
    goxiangqing(objectId,item){
      if(objectId!='no'){
         window.sessionStorage.setItem('tuijian8X',JSON.stringify(item))
         // this.$router.push({name:'tuijian8X',params:{'objectId':objectId}})
           window.open('#/HOME/tuijian8X/8/'+objectId)
      }else{
         // this.$router.push({name:'tuijian8X',params:{'objectId':objectId}})
           window.open('#/HOME/tuijian8X/8/'+objectId)
      }
...
https(page){
        ...
        that.$axios.get('boss/article', {
            params: {
            'searchkey':that.searchkey,'page':page,'recommend':1,'limit':limit10,'sessiontoken':sessiontoken
            }
          })
          .then(function (data) {
            if(data.status){
