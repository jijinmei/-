    ...
    <ul class="bai relative" style="z-index:20;">
      ...
      <i @click="goplace()" class="cursor layui-icon f14 absolute R20 T10" :class="{c222222:placeVal!='',cbbbbbb:placeVal==''}">{{place==false?'&#xe61a;':'&#xe619;'}}</i>  
       ...
       <li class="bai rf relative" style="width:380px;display:inline-block;z-index:10;">
        ...
        <div class="layui-form-item">
            <label class="layui-form-label w35">售價</label>
              ...
              <input v-model="priceVal" @click="goprice()" type="text" name="title" required readonly lay-verify="required" placeholder="请選擇售價" autocomplete="off" class="lf layui-input cursor center" style="width:300px;">
        </div>
...
<table class="layui-table margin0" id="laytable" style="table-layout:fixed;width:100%;"  v-if="datas!='no'">
      <thead>
        <tr  style="background-color:#ebfceb;">
         <th class="padding0" width='43'></th>
          ...
          <th class="padding0"  width='68'>售價<br><span class="f14">(萬元)</span></th>
  ...
  <br>
    <span class="cff4d00">{{item.user.nickname}}</span>
          ...
          <!-- 未封盤有更新过则显示更新时间黄色  否则显示发布时间黑色 -->
            <td class='padding0'>{{item.publishAt||item.createdAt}}</td>
        <!--   <td class='padding0' :class="{cfbb03b:item.cratedTimeStamp!=item.updatedTimeStamp}">{{item.cratedTimeStamp==item.updatedTimeStamp?timestampToTime(item.cratedTimeStamp):timestampToTime(item.updatedTimeStamp)}}</td> -->
         <!--  <td class='padding0' :class="{cff4d00:item.show==1,c36c748:item.show==1,c999999:item.show==0,cff0000:item.show==-1}"> -->
...
export default {
  ...
  data () {
    return {dengpao:false,//删除是否亮灯
        ...
        priceItem:['不限','200萬或以下','$200-$400萬','$400-$600萬','$600-$800萬','$800-$1000萬','$1000-$1200萬','$1200萬以上'],//状态的内容列表
        pricemin:'',
        pricemax:'',
        priceSou:[
                {'price_min':'','price_max':''},
  ...
  //获取区域
    ...
    goxiangqing(objectId){
      this.$router.push({name:'soupan2X',params:{'index':2,'objectId':objectId}})
    },
    // 价格
     priceclick(index,e){
      this.price=false
      this.priceVal=$(e.currentTarget).html()
      this.pricemin=this.priceSou[index].price_min
      this.pricemax=this.priceSou[index].price_max
      this.pages(1)
   ...
   var area=(this.shi.indexOf(',')!=-1?this.shi:(this.shi!='不限'?this.shi:(this.sheng=='不限'?'':this.sheng)))
    var type=(this.typeVal=='不限'?'':this.typeVal)
        that.$axios.get('boss/agent', {
            params: {
            'searchkey':that.searchkey,'page':page,'limit':limit10,'sessiontoken':sessiontoken,'category':'sell','type':type,'area':area,'show':show,'status':status,'price_min':that.pricemin,'price_max':that.pricemax
            }
          })
          .then(function (data) {
            if(data.status){
    ...
    var status=(this.statusIdx==0?'':(this.statusIdx==1||this.statusIdx==4?1:(this.statusIdx==2||this.statusIdx==5?0:-1)))
      ...
      var type=(this.typeVal=='不限'?'':this.typeVal)
        that.$axios.get('boss/agent', {
            params: {
            'searchkey':that.searchkey,'page':1,'limit':limit10,'sessiontoken':sessiontoken,'category':'sell','type':type,'area':area,'show':show,'status':status,'price_min':that.pricemin,'price_max':that.pricemax
            }
          })
          .then(function (data) {
            if(data.status){
