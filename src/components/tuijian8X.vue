<template>
<div class="youceng" style="height:950px;">
<!-- 返回 -->
        <div class="h70 relative f18 c999999">
            ...
            <!-- 创建 -->
            <!-- <span v-if="objectId=='no'" class="czjz R00 f18 cffffff w70 h40 lh40 center cursor" :class="{bg36c748:title!=''&&searchkey!='',bg999999:title==''||searchkey==''}">保存</span> -->
            <!-- 修改 -->
            <!-- <span v-if="objectId!='no'" class="czjz R00 f18 cffffff w70 h40 lh40 center cursor" :class="{bg36c748:title!='',bg999999:title==''}">保存</span> -->
        </div>
<!-- 表格 -->
<div class="bai pT16 pB05 pL16 pR16" style="width:748px;" >
<!-- <p class="f18 c222222 pT15">標題</p>
...
<!-- 创建的时候的样式 -->
<p class="f18 c222222" v-if="objectId=='no'">搜索並勾選對應的商店</p>
<form class="layui-form mT15" action="" v-if="objectId=='no'" onsubmit='return false'>
  <div class="layui-form-item">
    <div class="layui-input-block margin0">
      <input @keyup.enter="sousuo()" v-model="searchkey" type="text" name="title" lay-verify="title"   autocomplete="off" placeholder="請輸入關鍵字進行搜索" class="layui-input f18 L20 h40 lf" style="width:675px;">   
...
<table class="layui-table margin0" id="laytable" style="table-layout:fixed;width:100%;"  v-if="datas!='no'">
      <thead>
        <tr  style="background-color:#ebfceb;">
          ...
          <th class="padding0"  width='268'>商店名</th>
          <th class="padding0"  width='100'>分類</th>
          <th class="padding0"  width='178'>时间</th>
        </tr> 
      </thead>
      <tbody>
        <tr v-for="(item,index) in datas" >
          <td class='padding0 cursor center'>
             <!-- <img @click="tocheck($event,item.objectId)" :ids="item.objectId" srcs="../assets/img/wx.png"  src="../assets/img/wx.png" alt="" class="w28 dengp"> -->
              <img @click="tocheck($event,item.objectId)" :ids="item.objectId"  :srcs="item.recommend>0?'../assets/img/yx.png':'../assets/img/wx.png'"  :src="item.recommend>0?require('../assets/img/yx.png'):require('../assets/img/wx.png')" class="w28 dengp">
...
<!-- 表格:修改的时候的表格样式pT40 -->
<p class="f18 c222222  pB15 pL16 bai" v-if="objectId!='no'">對應的商店</p>
<div class="bai pB16 pL16 pR16 relative" style="width:748px;z-index:10;" v-if="objectId!='no'">
<table class="layui-table margin0" id="laytable" style="table-layout:fixed;width:100%;"  v-if="datas2!='no'">
      <thead>
        <tr  style="background-color:#ebfceb;">
          ...
          <th class="padding0"  width='376'>商店名</th>
          <th class="padding0"  width='180'>分類</th>
        </tr> 
      </thead>
      <tbody>
        <!-- <tr v-for="(item,index) in datas" > -->
        <tr>
          ...
          <td class='padding0'>{{datas2.section}}</td>
           <!-- <td class='padding0'>{{datas2.createdAt}}</td> -->
        </tr>
      </tbody>
</table>
...
export default {
  ...
  methods:{
           ...
           if($(e.currentTarget).attr('srcs')=='../assets/img/wx.png'){
            ...
            $.ajax({
                headers:{'token':sessiontoken},
                 type: "POST",
                 url: Boss+'boss/article/'+objectId+'/recommend',
                 data:{ 
                  'action':1,
   ...
   https(page){
        var that=this
        // 取得检举数据
        that.$axios.get('boss/article/'+this.objectId, {
            // params: {
    ...
    // 返回上一级
  goback(){
    this.$router.push({name:'tuijian8'})
  }
  ...
  if(this.objectId!='no'){
     // var bb=JSON.parse(window.sessionStorage.getItem('tuijian8X'))
     //     this.title=bb.title
    // this.pages(1)
    // this.pages(1)
    this.https(1)
